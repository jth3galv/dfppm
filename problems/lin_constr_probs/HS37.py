...
class HS37(LinConstrProb):
    """
    represents the problem HS37
     min_x - x1x2x3
      s. t. - 72 + x1 + 2x2 + 2x3 <= 0
            - x1 - 2x2 - 2x3 <=0
            0 <= x1 <= 42
    ...
    def __init__(self):
        ...
        self.__xstar = np.array((24, 12., 12.)).reshape(-1, 1)

        self.__m = 8
        self.__n = 3

        self.__b = np.array((72, 0, 42, 42, 42, 0, 0, 0), dtype=np.float).reshape(-1, 1)
        self.__A = np.zeros((self.__m, self.__n))

        self.__A[0] = np.array((1, 2, 2))
        self.__A[1] = np.array((-1, -2, -2))

        # box constraints
        for i in range(self.__n):
            self.__A[i + 2, i] = 1
        for i in range(self.__n):
            self.__A[i + 2 + + self.__n, i] = -1
