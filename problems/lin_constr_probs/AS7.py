...
class AS7(LinConstrProb):
    """
    represents the problem
     min_x sum_{i=1}^{n} xi**2
      s. t.
    ...
    def __init__(self, n: int):
        self.__n = n
        self.__x0 = np.ones((self.n, 1)) * 3
        self.__xstar = np.zeros_like(self.__x0)
    ...
    def f(self, x)->float:
        return float(np.sum((x.squeeze()) ** 2))
