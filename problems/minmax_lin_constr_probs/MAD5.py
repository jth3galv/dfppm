...
class MAD5(LinConstrProb):

    def __init__(self):
        self.__x0 = np.array((-1, 3), dtype=np.float).reshape(-1, 1)

        self.__n = 2
        self.__m = 1
        self.__b = np.array((-1,), dtype=np.float).reshape(-1, 1)
        self.__A = np.zeros((self.__m, self.__n))

        self.__A[0] = np.array((0.9, -1), dtype=np.float)
